...
float Ad[3][3] = {{0.5085,0.2, 0.0131},{-0.1075, 0.9954, 0.1185},{0,0,1}};
float Bd[3] = {0.0131, 0.1185, 0};
...
float k1d=-0.6271 ;
float k2d=2.7978;
float Ld[3] = {1.1924, 3.6273, 2.6556};
...
//  Serial.print(",");
  ...
  uk=(ref*Nu)+(-k1d*(xek[0]-Nx[0]*ref)-k2d*(xek[1]-Nx[1]*ref)-xek[2]);
  if(uk>160) {uk=160;}
  else if(uk<62) {uk=62;}
  xek1[0]=(uk*Bd[0])+(xk-(xek[0]*Csensor[0]+xek[1]*Csensor[1]+xek[2]*Csensor[2]))*Ld[0]+(xek[0]*Ad[0][0]+xek[1]*Ad[0][1]+xek[2]*Ad[0][2]);
  xek1[1]=(uk*Bd[1])+(xk-(xek[0]*Csensor[0]+xek[1]*Csensor[1]+xek[2]*Csensor[2]))*Ld[1]+(xek[0]*Ad[1][0]+xek[1]*Ad[1][1]+xek[2]*Ad[1][2]);
  xek1[2]=(uk*Bd[2])+(xk-(xek[0]*Csensor[0]+xek[1]*Csensor[1]+xek[2]*Csensor[2]))*Ld[2]+(xek[0]*Ad[2][0]+xek[1]*Ad[2][1]+xek[2]*Ad[2][2]);
